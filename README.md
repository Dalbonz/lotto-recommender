...
GitHub Actions가 매일 자동으로 최신 당첨번호를 수집해서 저장하고, GitHub Pages로 배포된다.
...
│   └── fetch_win720.py         # 연금복권720+ 결과 페이지 파싱으로 수집
└── .github/workflows/
    ├── update-data.yml         # 매일 자동 실행: 당첨번호 수집 → 커밋
    └── deploy-pages.yml        # main 브랜치 push 시 GitHub Pages 배포
```

## 자동 업데이트는 어떻게 동작하나

- `update-data.yml`이 매일 (UTC 13:00 = 한국시간 22:00) GitHub Actions에서 실행됩니다.
- 로또 6/45는 동행복권이 공개한 JSON 조회 API(`common.do?method=getLottoNumber`)를 사용합니다.
  이 API는 별도 키 없이 무료로 쓸 수 있는, 널리 알려진 공개 엔드포인트입니다.
- 연금복권720+는 **공식 API가 없어서** 공개 결과 페이지(HTML)를 받아 텍스트 패턴으로
  당첨번호를 추출하는 방식을 씁니다. 페이지 구조가 바뀌면 파싱이 실패할 수 있는데,
  그 경우 기존 데이터를 덮어쓰지 않고 조용히 건너뛰도록 만들어뒀습니다
  (Actions 로그에서 `[win720]` 경고로 확인 가능).
